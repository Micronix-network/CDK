...
public class ResultListTable extends CrudView {
    ...
    protected void renderBody(Writer writer) throws IOException {
        ...
        if (values != null) {
            ...
            for (String field_key : fields.keySet()) {
                ...
                if (toList != null) {
                    List sfaRoles = StringUtil.stringToList(toList.roles());
                    if (toList.listed() && checkRole(roles, sfaRoles) && !toList.hidden()) {
                        drawTH(writer, toList, orderby, field_key);
                        colSpan++;
                    }
                }
            }
            ...
            for (Object row : values.getResult()) {
                ...
                for (String field_key : fields.keySet()) {
                    ...
                    if (toList != null) {
                        List sfaRoles = StringUtil.stringToList(toList.roles());
                        if (toList.listed() && checkRole(roles, sfaRoles) && !toList.hidden()) {
                            String rule = toList.cellRule();
                            if(rule.equals("nill")){
                                SelectRenderer select = field.getAnnotation(SelectRenderer.class);
                                if(select!=null){
                                    rule=select.map();
                                }
                            }
                            String drag = toList.draggable();
                            boolean down = toList.downlink();
    ...
    private void drawTH(Writer writer, ToList toList, List<String> orderby, String field_key) throws IOException {
        String cardId = (String) stack.findValue("cardId");
