...
<#if target??>
	<@s.i18n name="${targetClass}">	
		<div class="${cardId}_form_title" style="">
			<#if operation='update' || operation='view'>
				<@s.text name="${targetClassName}.objName"/> : ${target}
			</#if>
			<#if operation='insert'>
				<@s.text name="${targetClassName}.objName"/>
			</#if>
		</div>
		<#if operation='update'>
	...
	<div id="${cardId}_${childName}_${uiid}_tool_bar" class="${cardId}_form_title"  style="margin-bottom: 4px;height:35px">
		...
		<tr>
			...
			<button id="${cardId}_${childName}_${uiid}_new_child_button" class="btn btn-small btn-primary" style=""><@s.text name="${targetClassName}.${childName}.new.button">${targetClassName}.${childName}.new.button</@s.text></button>
			</td>
		</tr>
		</table>	
	</div>
	...
	<div id="${cardId}_remove_child_confirm_dialog" class="${cardId}_topDownDialog">
		...
		<div class="diag_message"><@s.text name="${targetClassName}.${childName}.remove.confirm"/></div>
...
<#else>
	...
	<div class="message splash" style="margin-top: 50px; display: none">
		<div class="title">
			<@s.text name="crude.form.msg.title" />
		</div>
		<div class="body">
			<@s.text name="crude.form.msg.message" />
		</div>
	</div>	
</#if>
