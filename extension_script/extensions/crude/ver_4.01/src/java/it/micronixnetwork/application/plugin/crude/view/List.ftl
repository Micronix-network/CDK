...
<#assign js_row_click=action.getCardParam('row_click_event')!""/>
<style>
.highlight{
...
function ${cardId}_on_row_click($row){
	try{
		${js_row_click};
	}catch(err)
	{}
...
function ${cardId}_addSelected($row){
...
$(document).ready(function(){
	...
	//$('#${cardId}_${targetClassName}.${cardId}_gbtable tbody tr:odd').addClass('odd');
	//$('#${cardId}_${targetClassName}.${cardId}_gbtable tbody tr:even').addClass('even');
...
<#if only_list_flag='false'>		
	$('.${cardId}_${targetClassName}_resultRow').click(function(){
		  ...
		  <#else>
			...
			$('#${cardId}_${targetClassName}_view_ui_get_obj_action').show();
			$('#${cardId}_${targetClassName}_view_ui_update_object_action_content').html('<span class="${cardId}_event_message animated flash"><@s.text name="crude.form.rquired"/></span>');
	...
	<#if only_list_flag='true' && autoselect_flag='true'>
