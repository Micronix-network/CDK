...
public abstract class EditAction extends CrudAction {
    ...
    protected void validInput() {
        ...
        setLocalProvider(target.getName());
    ...
    private void checkValid(Class target) {
        if (target != null) {
            ...
            for (Field field : fields) {
                if (field.getModifiers() == 1) {
                    ...
                    if (validF != null) {
                        if (!validField(target, validF, field)) {
                            return;
                        }
                    }
                }
    ...
    private boolean validField(Class target, ValidField validF, Field idfield) {
        String type = validF.type();
        String rule = validF.rule();

        String filedLabel = null;
        filedLabel = getText(target.getSimpleName() + "." + idfield.getName(), idfield.getName());
        if (filedLabel == null) {
    ...
    private boolean checkVoid(Class target) {
        if (target != null) {
            Field[] fields = target.getFields();
            for (Field field : fields) {
                if (field.getModifiers() == 1) {
                    ...
                    if (validF != null) {
                        if (!validF.empty()) {
                            if (ExprValidator.validVoid(objState.get(field.getName()))) {
                                String filedLabel = null;
                                filedLabel = getText(target.getSimpleName() + "." + field.getName(), field.getName());
                                if (filedLabel == null) {
                                    filedLabel = field.getName();
                                }
        ...
        try {
            ...
            if (msg.startsWith("Duplicate entry")) {
                ...
                if (fieldName != null) {
                    ...
                    filedLabel = getText(target.getSimpleName() + "." + fieldName, fieldName);
                    if (filedLabel == null) {
